...
//[SIMT-SWH-110811]{
static int g_nPwrKeyDown = 0;
extern int IsPwrkeyDown(void)
{
    return g_nPwrKeyDown;
}
EXPORT_SYMBOL(IsPwrkeyDown);
//[SIMT-SWH-110811]}
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	printk(KERN_ERR "[keypad] report_hs_key(%ul, %ul) Enter \n",key_code, key_parm);
...
//[SIMT-SWH-110811]{
    if(KEY_POWER == key)
    {
        g_nPwrKeyDown = ( key_code != HS_REL_K) ? 1 : 0;  
    }
	printk(KERN_ERR "[keypad] report_hs_key() Exit key<%d>, g_nPwrKeyDown<%d>\n",key, g_nPwrKeyDown);
//[SIMT-SWH-110811]}
