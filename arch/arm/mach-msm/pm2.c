...
extern uint32_t smsm_check_reboot_data(uint32_t income_reason);//[SIMT SWH 202110907]

static void msm_pm_power_off(void)
{
    //int count = 0;//[SIMT SWH 202110907]
    int nLocalReason = 0;//[SIMT SWH 202110907]

	msm_rpcrouter_close();

    //[SIMT SWH 202110907] {
    nLocalReason = smsm_check_reboot_data(0);
    /*
    for(count = 1; count>0; count--)
    {
        pr_err("[SMD]pm2 %s: wait to reset chip, count<%d> nLocalReason<0x%x>, restart_reason<0x%x>\n",__func__, count, nLocalReason, restart_reason);          
        //msleep(200);
    }
    */
	msm_proc_comm(PCOM_POWER_DOWN, &nLocalReason, 0);/*SWH*/
    //[SIMT SWH 202110907] }
...
static void msm_pm_restart(char str, const char *cmd)
{
   //int count = 0;//[SIMT SWH 202110907]
	...
	msm_rpcrouter_close();
    //[SIMT SWH 202110907] {
    restart_reason = smsm_check_reboot_data(restart_reason);/*SWH*/

    /*
    for(count = 1; count>0; count--)
    {
        pr_err("[SMD]pm2 %s: wait to reset chip, count<%d> restart_reason<0x%x>\n",__func__, count, restart_reason);          
        msleep(200);
    }
    */
    //[SIMT SWH 202110907] }
