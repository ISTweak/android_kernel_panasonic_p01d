...
static const unsigned int surf_keymap[] = {
#ifdef CONFIG_BIGBANG_KEY
	/*keymap below is used for bigbang project*/
	KEY(0, 0, KEY_VOLUMEDOWN),
	KEY(0, 1, KEY_VOLUMEUP),//KEY_BACK//KEY_DOWN
	...
	KEY(1, 0, KEY_VOLUMEDOWN),//KEY_LEFT
	KEY(1, 1, KEY_FOCUS),/*qiukj add for double action shutter key on bigbang*///KEY_1
	KEY(1, 2, KEY_CAMERA),//KEY_HOME//KEY_SEND
	KEY(1, 3, KEY_4),
	KEY(1, 4, KEY_CLEAR),
	KEY(1, 5, KEY_MSDOS),
	KEY(1, 6, KEY_SPACE),
	...
	KEY(1,  1, KEY_HOME),//KEY_SEND
        ...
        //[SIMT-huangli-20110903] no global keyboard for ponyo
#if 0
	KEY(0, 3, KEY_RIGHT),
	KEY(0, 4, KEY_ENTER),
	KEY(0, 5, KEY_L),
...
//		.default_trigger = "timer", // [SIMT-qiukejun-110726] used to accomplish blinking led
	},
	{
		.name		= "red",
		.max_brightness = 40,
...
//
#ifdef CONFIG_BIGBANG //bigbang
static uint32_t camera_off_gpio_table[] = {
	GPIO_CFG(0,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /*PWDN-VAG  */
	GPIO_CFG(31, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /*RST  */
	GPIO_CFG(143,0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* PWDN */
	GPIO_CFG(33, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* TORCH_EN */
	//GPIO_CFG(3,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT3 */
	GPIO_CFG(4,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT4 */
	GPIO_CFG(5,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT5 */
	GPIO_CFG(6,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT6 */
...
static uint32_t camera_on_gpio_table[] = {
	...
	GPIO_CFG(31, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* RST */
	GPIO_CFG(143,0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* PWDN */
	GPIO_CFG(33,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_16MA), /* RSTDW */
	//GPIO_CFG(3,  1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT3 */
	GPIO_CFG(4,  1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT4 */
	GPIO_CFG(5,  1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT5 */
	GPIO_CFG(6,  1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* DAT6 */
...
//	{ GPIO_CFG(126, 1, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_4MA), "dtv_vsync" }, //Yuhaipeng 20110506
...
static int display_common_power(int on)
{
	...
	else
	{
		...
		rc = vreg_set_level(vreg_ldo20, 1800); // [SIMT-qiukejun-111019] change from 1500 to 1800 according to hw requirement
	}
	if (rc) {
	...
	if (machine_is_msm7x30_fluid()) {
		...
		//[SIMT-qiukejun-20110718] adjust voltage from 2.8 v to 3 v for LCM lg4573
...
#ifdef  CONFIG_BIGBANG_GPIO112_TP_CANNOT_USE
...
{
    // wanggang add for irda 20110620 start
    #if 0
	int rc;
  
  vreg_gp4 = vreg_get(NULL, "gp4");
  rc = vreg_set_level(vreg_gp4, 3000);
  if (rc) {
   pr_err("%s: vreg_set_level failed \n", __func__);
   vreg_disable(vreg_gp4);
   return;
  ...
  if (rc) {
   ...
   return;
	...
	gpio_set_value(IRDA_VLED_EN, 1);
  #endif
    // wanggang add for irda 20110620 end
...
//	vreg_lvs0 = vreg_get(NULL, "lvsw0"); // [SIMT-qiukejun-111019] remove according hw requirement
...
//	if (vreg_12 && vreg_16 && vreg_lvs0) {
	if (vreg_12 && vreg_16) {
		if (on == CYTTSP_POWER_ON || on == ATMEL_POWER_ON) {
			...
			gpio_tlmm_config(GPIO_CFG(GPIO_CTP_IRQ,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
            #if 0/*TP normal work.*/
			gpio_tlmm_config(GPIO_CFG(GPIO_I2C_SDA, 0, GPIO_CFG_OUTPUT,GPIO_CFG_PULL_UP, GPIO_CFG_16MA), GPIO_CFG_ENABLE);
			gpio_tlmm_config(GPIO_CFG(GPIO_I2C_SCL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_16MA), GPIO_CFG_ENABLE);
			#else/*TP don't work while pown on, need to receive resume msg.*/
...
#if 0
			//vreg_set_level(vreg_lvs0, 0);
			vreg_disable(vreg_lvs0);
			//vreg_set_level(vreg_12, 0);
			vreg_disable(vreg_12);
			//vreg_set_level(vreg_16, 0);
			vreg_disable(vreg_16);
#endif
...
#if 0//cypress's CTP
static int cyttsp_i2c_wakeup(int on)
{
	return ctp_power(on);
}
...
#elif defined(CONFIG_BIGBANG)
	{GPIO_CFG(43, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "headset_key"},
#else
	Program Error!
#endif
};
