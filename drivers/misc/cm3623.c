...
static int cm3623_write_reg(struct cm3623_sensor_data *sd, u16 addr, u8 val) {
	int i, rc;
    
	sd->client->addr = addr;
	
    for (i = 0; i < CM3623_RETRY_NUM; i++) {
...
static int cm3623_read_reg(struct cm3623_sensor_data *sd, u16 addr) {
	int i, rc;
...
static void cm3623_report(struct cm3623_sensor_data *sd, int type)
{
	...
	do {
		if (type & CM3623_ALS_BIT) {
			rc = cm3623_read_reg(sd, CM3623_ALS_RD_LSB);
			if (rc < 0) 
				break;
			rc &= 0xFF;
			val = rc;
			rc = cm3623_read_reg(sd, CM3623_ALS_RD_MSB);
			if (rc < 0) 
				break;
			rc &= 0xFF;
			val |= (rc << 8);
		...
		if (type & CM3623_PS_BIT) {
			rc = cm3623_read_reg(sd, CM3623_PS_RD);
			if (rc < 0) 
				break;
			sd->distance[0] = rc;
...
static int cm3623_cfg_tigger(struct cm3623_sensor_data *sd) {
	...
	if (sd->ps_threshold) {
        rc = cm3623_write_reg(sd, CM3623_PS_THD_WR, sd->ps_threshold);
		if (sd->gpio_int) {
			rc = gpio_get_value(sd->gpio_int);
...
static int cm3623_config(struct cm3623_sensor_data *sd)
{
	...
	val |= (0 << 4);  // THD_ALS
	val |= (0 << 6);  // GAIN_ALS
	rc = cm3623_write_reg(sd, CM3623_ALS_WR, val);
    if (rc < 0) {
        goto config_failed;
    }
    
	rc = cm3623_write_reg(sd, CM3623_PS_THD_WR, sd->ps_threshold); // THD_PS
    if (rc < 0) {
        goto config_failed;
    }
	...
	val |= (3 << 4);  // IT_PS    
	val |= (0 << 6);  // DR_PS    
	rc = cm3623_write_reg(sd, CM3623_PS_WR, val);
    if (rc < 0) {
        goto config_failed;
    }
...
static int cm3623_up_ps(struct cm3623_sensor_data *sd)
{
	...
	//printk("%s\n", __func__);
    rc = cm3623_write_reg(sd, CM3623_PS_WR, 0x30);
	mdelay(10);
...
static int cm3623_shutdown_ps(struct cm3623_sensor_data *sd)
{
    ...
    rc = cm3623_write_reg(sd, CM3623_PS_WR, 0x31);
	mdelay(10);
...
static int cm3623_up_als(struct cm3623_sensor_data *sd)
{
    ...
    rc = cm3623_write_reg(sd, CM3623_ALS_WR, 0x02);
	mdelay(10);
...
static int cm3623_shutdown_als(struct cm3623_sensor_data *sd)
{
    ...
    rc = cm3623_write_reg(sd, CM3623_ALS_WR, 0x03);
	mdelay(10);
...
static int cm3623_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	for (i=0; i<3; i++) {
		rc = cm3623_init_addr(sensor_data);
		if (rc < 0) {
			printk(KERN_ERR "cm3623 init failed\n");
		goto probe_err_cfg;
		}
	}
