...
static ssize_t mtd_read(struct file *file, char __user *buf, size_t count,loff_t *ppos)
{
	...
	while (count) {
		...
		//Add by hxh for modem partition op,2011-07-23
		case MTD_MODE_MODEM:
		{
			//printk("///////////////////////////////mtd_modem_read\n");
			ret = mtd->modem_read(mtd, *ppos, len, &retlen, kbuf);
			break;
		}
		//Add end
		default:
	...
	while (count) {
		...
		case MTD_MODE_RAW:
		{
			...
			break;
              ...
              //Add by hxh for modem partition op,2011-07-23
              case MTD_MODE_MODEM:
			ret = (*(mtd->modem_write))(mtd, *ppos, len, &retlen, kbuf);  	
			break;  	
              //Add end
		default:
	...
	uint32_t __user *retp)
{
	...
	//Modify by hxh for modem partition op,2011-07-23
	if(mfi->mode != MTD_MODE_MODEM)
	    ret = mtd->write_oob(mtd, start, &ops);
	else
	    ret = mtd->modem_write_oob(mtd, start, &ops);	
	//Modify end
...
//Modify end	
{
	...
	if(mfi->mode != MTD_MODE_MODEM)
	     ...
	     ret = mtd->modem_read_oob(mtd, start, &ops);	
	//Modify end
...
static int mtd_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case MEMGETBADBLOCK:
	{
		...
		{
		   if (!mtd->modem_block_isbad)
			ret = -EOPNOTSUPP;
		   else
			return mtd->modem_block_isbad(mtd, offs);
		}
		//Modify end
		break;
	}
	...
	case MEMSETBADBLOCK:
	{
		...
		{
		   if (!mtd->modem_block_markbad)
			ret = -EOPNOTSUPP;
		   else
			return mtd->modem_block_markbad(mtd, offs);
		}
		//Modify end
		break;
	}
	...
	case MTDFILEMODE:
	{
		...
		//Add by hxh for modem partition op,2011-07-23	
              case MTD_MODE_MODEM:
			//printk("///////////////////////////////ioctl value %ld\n",arg);
			if (!mtd->modem_read_oob || !mtd->modem_write_oob)
			{
			       if(!mtd->modem_read_oob)
				   	printk("///////////////////////////////modem read oob null\n");
				if(!mtd->modem_write_oob)  
					printk("///////////////////////////////modem write oob null\n");
				return -EOPNOTSUPP;
			}
			//printk("*********************************ioctl value %ld\n",arg);
			mfi->mode = arg;
