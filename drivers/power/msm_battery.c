...
int battery_misc_ioctl (struct inode *inode, struct file *flip, unsigned int cmd, unsigned long arg)
{
    ...
    else if (BATT_IOCTL_RESET_CHIP == cmd)
    {
        ...
        unsigned nIn = (unsigned)arg;

        smsm_get_reset_reason(&nResetTypeOut);
        pr_err("[batt] %s: BATT_IOCTL_RESET_CHIP nIn<%d> nResetTypeOut<%d>", __func__, nIn, nResetTypeOut );  

        if(nResetTypeOut != nIn)
        {
            smsm_set_reset_reason(nIn);
        }
...
void UpdateResetReason(void)
{
    ...
    if(RESET_BASE != nResetTypeOut)
    {
        pr_err("[batt] %s:  Clear the reason if not a default reset", __func__ );  
